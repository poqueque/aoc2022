import 'dart:io';

import 'package:aoc2022/extensions/extensions.dart';

import '../main.dart';
import '../utils/coor.dart';

class Day22 extends Day {
  @override
  bool get completed => true;

  Map<Coor, String> map = {};
  Map<int, Coor> minX = {};
  Map<int, Coor> maxX = {};
  Map<int, Coor> minY = {};
  Map<int, Coor> maxY = {};

//  String instructions = "10R5L5R10L4R5L5";
  String instructions =
      "";
  List<dynamic> inst = [];
  Coor? start;

  @override
  init() {
    map.clear();
    inst.clear();
    var y = 1;
    for (var line in inputList) {
      var x = 1;
      for (var c in line.chars) {
        if (c != " ") {
          if (start == null) start = Coor(x, y);
          map[Coor(x, y)] = c;
          if (minY[x] == null) minY[x] = Coor(x, y);
          if (minX[y] == null) minX[y] = Coor(x, y);
          maxY[x] = Coor(x, y);
          maxX[y] = Coor(x, y);
        }
        x++;
      }
      y++;
    }
    var value = 0;
    for (var i in instructions.chars) {
      var n = int.tryParse(i);
      if (n == null) {
        inst.add(value);
        inst.add(i);
        value = 0;
      } else {
        value *= 10;
        value += n;
      }
    }
    if (value > 0) inst.add(value);

    print(instructions);
    print(inst);
  }

  List<Direction> D = [
    Direction.right,
    Direction.down,
    Direction.left,
    Direction.up
  ];
  List<String> icon = [">", "v", "<", "^"];
  List<String> position = ["D", "V", "C", "A"];

  int facing = 0;
  late Coor p;

  @override
  part1() {
    p = start!;
    for (var i in inst) {
      move(i);
    }
/*
    print(icon[facing]);
    CoorMap m = CoorMap();
    m.map = map;
    m.printMap(spaces: false);
    print("");
*/

    print(p);
    return 1000 * p.y + 4 * p.x + facing;
  }

  Map<Coor, String> mapA = {};
  Map<Coor, String> mapB = {};
  Map<Coor, String> mapC = {};
  Map<Coor, String> mapD = {};
  Map<Coor, String> mapE = {};
  Map<Coor, String> mapF = {};
  late Map<Coor, String> currentMap;
  Coor p2 = Coor(1, 1);

  @override
  part2() {
    init();
    currentMap = mapA;
    facing = 0;

    for (int y = 1; y <= 50; y++)
      for (int x = 51; x <= 100; x++) mapA[Coor(x - 50, y)] = map[Coor(x, y)]!;
    for (int y = 1; y <= 50; y++)
      for (int x = 101; x <= 150; x++)
        mapB[Coor(x - 100, y)] = map[Coor(x, y)]!;
    for (int y = 51; y <= 100; y++)
      for (int x = 51; x <= 100; x++)
        mapC[Coor(x - 50, y - 50)] = map[Coor(x, y)]!;
    for (int y = 101; y <= 150; y++)
      for (int x = 1; x <= 50; x++) mapE[Coor(x, y - 100)] = map[Coor(x, y)]!;
    for (int y = 101; y <= 150; y++)
      for (int x = 51; x <= 100; x++)
        mapD[Coor(x - 50, y - 100)] = map[Coor(x, y)]!;
    for (int y = 151; y <= 200; y++)
      for (int x = 1; x <= 50; x++) mapF[Coor(x, y - 150)] = map[Coor(x, y)]!;

    for (var i in inst) {
      moveCube(i);
      print("Move: $i");
//      if (i is int) printMap2();
      print("");
    }

    printMap2();

    if (currentMap == mapA) print("Map A");
    if (currentMap == mapB) print("Map B");
    if (currentMap == mapC) print("Map C");
    if (currentMap == mapD) print("Map D");
    if (currentMap == mapE) print("Map E");
    if (currentMap == mapF) print("Map F");
    print(p2);
    var finalX = p2.x;
    var finalY = p2.y;
    if (currentMap == mapA) finalX += 50;
    if (currentMap == mapB) finalX += 100;
    if (currentMap == mapC) finalX += 50;
    if (currentMap == mapC) finalY += 50;
    if (currentMap == mapD) finalX += 50;
    if (currentMap == mapD) finalY += 100;
    if (currentMap == mapE) finalY += 100;
    if (currentMap == mapF) finalY += 150;
    print("($finalX,$finalY) - $facing");

    return 1000 * finalY + 4 * finalX + facing;
  }

  void move(dynamic step) {
    if (step is String) {
      if (step == "R") facing = (facing + 1) % 4;
      if (step == "L") facing = (facing - 1) % 4;
    }
    if (step is int) {
      for (int i = 0; i < step; i++) {
        map[p] = icon[facing];
        var tryP = p.move(D[facing]);
        if (map[tryP] == null && D[facing] == Direction.right)
          tryP = minX[tryP.y]!;
        if (map[tryP] == null && D[facing] == Direction.down)
          tryP = minY[tryP.x]!;
        if (map[tryP] == null && D[facing] == Direction.left)
          tryP = maxX[tryP.y]!;
        if (map[tryP] == null && D[facing] == Direction.up)
          tryP = maxY[tryP.x]!;
        if (map[tryP] != "#") p = tryP;
      }
    }
  }

  void moveCube(dynamic step) {
    if (step is String) {
      if (step == "R") facing = (facing + 1) % 4;
      if (step == "L") facing = (facing - 1) % 4;
      currentMap[p2] = position[facing];
    }
    if (step is int) {
      for (int i = 0; i < step; i++) {
        currentMap[p2] = icon[facing];
        var tryP = p2.move(D[facing]);
        var tryMap = currentMap;
        var tryFacing = facing;
        //MAP A
        if (tryP.x == 0 && currentMap == mapA) {
          tryP.y = 51 - tryP.y;
          tryP.x = 1;
          tryMap = mapE;
          tryFacing = 0;
        }
        if (tryP.x == 51 && currentMap == mapA) {
          tryP.x = 1;
          tryMap = mapB;
          tryFacing = 0;
        }
        if (tryP.y == 0 && currentMap == mapA) {
          tryP.y = tryP.x;
          tryP.x = 1;
          tryMap = mapF;
          tryFacing = 0;
        }
        if (tryP.y == 51 && currentMap == mapA) {
          tryP.y = 1;
          tryMap = mapC;
          tryFacing = 1;
        }

        //MAP B
        if (tryP.x == 0 && currentMap == mapB) {
          tryP.x = 50;
          tryMap = mapA;
          tryFacing = 2;
        }
        if (tryP.x == 51 && currentMap == mapB) {
          tryP.y = 51 - tryP.y;
          tryP.x = 50;
          tryMap = mapD;
          tryFacing = 2;
        }
        if (tryP.y == 0 && currentMap == mapB) {
          tryP.y = 50;
          tryMap = mapF;
          tryFacing = 3;
        }
        if (tryP.y == 51 && currentMap == mapB) {
          tryP.y = tryP.x;
          tryP.x = 50;
          tryMap = mapC;
          tryFacing = 2;
        }

        //MAP C
        if (tryP.x == 0 && currentMap == mapC) {
          tryP.x = tryP.y;
          tryP.y = 1;
          tryMap = mapE;
          tryFacing = 1;
        }
        if (tryP.x == 51 && currentMap == mapC) {
          tryP.x = tryP.y;
          tryP.y = 50;
          tryMap = mapB;
          tryFacing = 3;
        }
        if (tryP.y == 0 && currentMap == mapC) {
          tryP.y = 50;
          tryMap = mapA;
          tryFacing = 3;
        }
        if (tryP.y == 51 && currentMap == mapC) {
          tryP.y = 1;
          tryMap = mapD;
          tryFacing = 1;
        }

        //MAP D
        if (tryP.x == 0 && currentMap == mapD) {
          tryP.x = 50;
          tryMap = mapE;
          tryFacing = 2;
        }
        if (tryP.x == 51 && currentMap == mapD) {
          tryP.y = 51 - tryP.y;
          tryP.x = 50;
          tryMap = mapB;
          tryFacing = 2;
        }
        if (tryP.y == 0 && currentMap == mapD) {
          tryP.y = 50;
          tryMap = mapC;
          tryFacing = 3;
        }
        if (tryP.y == 51 && currentMap == mapD) {
          tryP.y = tryP.x;
          tryP.x = 50;
          tryMap = mapF;
          tryFacing = 2;
        }

        //MAP E
        if (tryP.x == 0 && currentMap == mapE) {
          tryP.y = 51 - tryP.y;
          tryP.x = 1;
          tryMap = mapA;
          tryFacing = 0;
        }
        if (tryP.x == 51 && currentMap == mapE) {
          tryP.x = 1;
          tryMap = mapD;
          tryFacing = 0;
        }
        if (tryP.y == 0 && currentMap == mapE) {
          tryP.y = tryP.x;
          tryP.x = 1;
          tryMap = mapC;
          tryFacing = 0;
        }
        if (tryP.y == 51 && currentMap == mapE) {
          tryP.y = 1;
          tryMap = mapF;
          tryFacing = 1;
        }

        //MAP F
        if (tryP.x == 0 && currentMap == mapF) {
          tryP.x = tryP.y;
          tryP.y = 1;
          tryMap = mapA;
          tryFacing = 1;
        }
        if (tryP.x == 51 && currentMap == mapF) {
          tryP.x = tryP.y;
          tryP.y = 50;
          tryMap = mapD;
          tryFacing = 3;
        }
        if (tryP.y == 0 && currentMap == mapF) {
          tryP.y = 50;
          tryMap = mapE;
          tryFacing = 3;
        }
        if (tryP.y == 51 && currentMap == mapF) {
          tryP.y = 1;
          tryMap = mapB;
          tryFacing = 1;
        }

        if (tryMap[tryP] != "#") {
          currentMap = tryMap;
          p2 = tryP;
          facing = tryFacing;
          currentMap[p2] = position[facing];
        }
      }
    }
  }

  printMap2() {
    for (int y = 1; y <= 200; y++) {
      for (int x = 1; x <= 150; x++) {
        if (x > 50 && x <= 100 && y <= 50)
          stdout.write(mapA[Coor(x - 50, y)]);
        else if (x > 100 && x <= 150 && y <= 50)
          stdout.write(mapB[Coor(x - 100, y)]);
        else if (x > 50 && x <= 100 && y > 50 && y <= 100)
          stdout.write(mapC[Coor(x - 50, y - 50)]);
        else if (x > 50 && x <= 100 && y > 100 && y <= 150)
          stdout.write(mapD[Coor(x - 50, y - 100)]);
        else if (x <= 50 && y > 100 && y <= 150)
          stdout.write(mapE[Coor(x, y - 100)]);
        else if (x <= 50 && y > 150 && y <= 200)
          stdout.write(mapF[Coor(x, y - 150)]);
        else
          stdout.write(" ");
      }
      print("");
    }
  }
}
